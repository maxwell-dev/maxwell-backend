...
%%%-------------------------------------------------------------------
-module(maxwell_backend_registrar).
...
init([]) ->
  Ref = erlang:monitor(process, maxwell_backend_master_connector:get_pid()),
  State = #state{connector_ref = Ref, registered = false},
  maxwell_backend_master_connector:add_listener(self()),
...
handle_response(#register_backend_rep_t{}, State) ->
  lager:info("Backend was successfully registered!", []),
  State#state{registered = true};
handle_response(#error_rep_t{code = _, desc = _} = Rep, State) ->
  lager:info("Failed to register backend: ~p", [Rep]),
  send_cmd(?REGISTER_CMD, 1000),
  State#state{registered = false};
handle_response({error, {_, _}} = Error, State) ->
  lager:info("Failed to register backend: ~p", [Error]),
  send_cmd(?REGISTER_CMD, 1000),
  State#state{registered = false}.

build_public_endpoint() ->
  io_lib:format(
    "~p:~p", [maxwell_ip_resolver:resolve(), maxwell_server_config:get_port()]
  ).
